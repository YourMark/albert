...
| `albert/dashboard/stats` | filter | Append tiles to the Dashboard's stat row. Free seeds it with the two figures it can compute (enabled abilities, active connections); an add-on with call/failure/duration history appends its own `[ 'value' => string, 'label' => string ]` tiles rather than Free rendering a zero for data it doesn't retain. See `Dashboard::render_stat_row()` |
